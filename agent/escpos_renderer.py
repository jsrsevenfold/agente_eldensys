...
A partir da v0.2 o agente é um intermediário puro: nenhuma configuração
global de formatação é aplicada. O cliente (EldenSys) manda os comandos
exatamente como quer ver impressos — alinhamento, tamanho, margens,
fonte. Valores ausentes nos comandos caem em defaults conservadores
do próprio python-escpos.
"""
...
def render(payload: EscposPayload) -> None:
    ...
    O agente não aplica nenhuma transformação global — cada comando é
    enviado exatamente como vem do cliente.
    """
...
def _apply(p: Any, cmd: EscposCommand, profile: Profile) -> None:
    ...
    if t == "text":
        ...
        if cmd.font is not None:
            kwargs["font"] = cmd.font
        p.set(**kwargs)
        p.text((cmd.text or "") + "\n")
        # Reset pra valores padrão da impressora (alinhado à esquerda, sem
        # bold/underline, tamanho 1, fonte "a") — evita "vazar" estilo
        # pra próximas linhas que vierem sem campos explícitos.
        p.set(
            align="left",
            bold=False,
            underline=0,
